...
const AdvancedFilterPage = () => {
  ...
  const FilterSection = ({ title, items, category }) => (
    <div className="mb-8">
      <h3 className="text-lg font-bold mb-4 text-slate-500 flex items-center gap-2">
        {title}
      </h3>
    ...
    <MainLayout>
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <h1 className="text-2xl font-extrabold text-slate-500">Filtres Avancés</h1>
          <button 
        ...
        <div className="space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              <h3 className="text-lg font-bold mb-4 text-slate-500 flex items-center gap-2">
                <CalendarIcon className="w-5 h-5 text-orange-500" /> Choisir un jour
              </h3>
              <input 
                type="date" 
                value={filters.day}
                onChange={(e) => setFilters({...filters, day: e.target.value})}
                className="w-full p-4 rounded-xl border border-slate-200 bg-white text-slate-500 focus:ring-2 focus:ring-orange-500 outline-none transition-all shadow-sm"
              />
            </div>
            <div>
              <h3 className="text-lg font-bold mb-4 text-slate-500 flex items-center gap-2">
                <Clock className="w-5 h-5 text-orange-500" /> Heure précise
              </h3>
              <input 
                type="time" 
                value={filters.time}
        ...
        <div className="sticky bottom-8 mt-12 flex justify-center pb-20 lg:pb-0">
          <Button 
            onClick={handleApply}
            className="w-full max-w-md shadow-2xl py-4 text-lg rounded-2xl bg-orange-600 text-white hover:bg-orange-700 transition-colors flex items-center justify-center font-bold"
          >
