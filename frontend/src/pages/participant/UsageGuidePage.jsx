  ...
  return (
    <MainLayout>
      <div className="bg-slate-50 min-h-screen font-sans selection:bg-orange-100 selection:text-orange-900 pb-20">
        ...
        {/* HERO SECTION */}
        <section className="bg-slate-900 pt-32 pb-24 md:pt-48 md:pb-32 relative overflow-hidden">
           ...
           <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-orange-600 rounded-full blur-[160px] opacity-20 -mb-64"></div>
           
           <div className="max-w-5xl mx-auto px-6 text-center space-y-8 relative z-10">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-xl border border-white/10 rounded-full text-white font-black text-[10px] uppercase tracking-widest animate-in fade-in slide-in-from-top-4 duration-700">
                 <HelpCircle size={14} className="text-orange-500" />
                 Centre de Connaissances
              </div>
              <h1 className="text-5xl md:text-8xl font-black text-white tracking-tighter uppercase leading-none animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100">
                 Guide <span className="text-orange-500">QR-EVENT</span>
              </h1>
              <p className="text-slate-400 font-bold text-lg max-w-2xl mx-auto leading-relaxed animate-in fade-in slide-in-from-bottom-4 duration-700 delay-200">
                 Tout ce qu'il faut savoir pour maîtriser la plateforme de billetterie la plus innovante.
              </p>
           </div>
        </section>
        ...
        <div className="max-w-7xl mx-auto px-6 -mt-16 relative z-20 space-y-24">
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              {sections.map((section, idx) => (
                <div key={idx} className="bg-white rounded-[3rem] p-10 md:p-16 shadow-2xl shadow-slate-200/50 border border-slate-50 space-y-12">
                   ...
                   <div className="space-y-8">
                      {section.steps.map((step, sIdx) => (
                        <div key={sIdx} className="flex gap-6 group">
                           <div className="w-12 h-12 bg-slate-50 text-slate-300 rounded-2xl flex items-center justify-center font-black text-lg transition-colors group-hover:bg-slate-900 group-hover:text-white shrink-0">
                              {sIdx + 1}
                           </div>
           ...
           {/* CTA SECTION */}
           <section className="bg-orange-600 rounded-[4rem] p-12 md:p-24 text-center space-y-10 shadow-3xl shadow-orange-600/30 overflow-hidden relative">
              <Zap size={200} className="absolute -right-20 -bottom-20 text-white opacity-10 rotate-12" />
              <div className="space-y-4 relative z-10">
                 <h2 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter leading-none">Prêt à lancer <br/>votre événement ?</h2>
                 <p className="text-orange-100 font-bold text-lg max-w-md mx-auto">Rejoignez des milliers d'organisateurs qui font confiance à QR-EVENT.</p>
              </div>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 relative z-10">
                 <Link to="/login" className="w-full sm:w-auto px-12 py-5 bg-white text-orange-600 font-black text-[10px] uppercase tracking-widest rounded-2xl shadow-xl hover:scale-105 transition-transform active:scale-95">
                    Commencer maintenant
                 </Link>
                 <Link to="/events" className="w-full sm:w-auto px-12 py-5 bg-orange-700 text-white font-black text-[10px] uppercase tracking-widest rounded-2xl flex items-center justify-center gap-3 hover:bg-orange-800 transition-colors">
